...
- `packages/infra` is created when a Cloudflare, Prisma, or AWS deployment is enabled.
...
When `runtime=lambda`, the server targets AWS Lambda and is packaged from `apps/server/src/lambda.ts`. An AWS server or web deployment generates the matching ECS, Lambda, VPC, and Aurora resources in `packages/infra/alchemy.run.ts`.
...
```json
{
  ...
  "runtime": "<bun|node|workers|lambda|none>",
  ...
  "addons": ["<pwa|tauri|electrobun|starlight|fumadocs|biome|lefthook|husky|mcp|turborepo|nx|vite-plus|ultracite|oxlint|eslint|opentui|wxt|skills|evlog|axiom|turnstile|none>"] ,
  "examples": ["<ai|todo|none>"] ,
  "auth": <"better-auth"|"clerk"|"none">,
  "packageManager": "<bun|pnpm|npm>",
  "dbSetup": "<turso|neon|prisma-postgres|planetscale|aurora|mongodb-atlas|supabase|d1|docker|none>",
  "api": "<none|trpc|orpc>",
  "webDeploy": "<cloudflare|prisma|aws|docker|vercel|none>",
  "serverDeploy": "<cloudflare|prisma|aws|docker|vercel|none>"
}
```
...
- `packages/infra`: Alchemy infrastructure for Cloudflare, Prisma, and/or AWS deployments
...
- **Deploy**: Cloudflare, Prisma, or AWS deployment adds one `packages/infra/alchemy.run.ts` containing the selected application and database resources
