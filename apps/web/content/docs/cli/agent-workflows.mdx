...
- Accepted values: `runtime` is one of `bun`, `node`, `workers`, `lambda`, or `none`; `dbSetup` is one of `turso`, `d1`, `neon`, `supabase`, `prisma-postgres`, `planetscale`, `aurora`, `mongodb-atlas`, `docker`, or `none`; `webDeploy` and `serverDeploy` are one of `cloudflare`, `prisma`, `aws`, `docker`, `vercel`, or `none`.
...
- When the database-consuming application deploys through Alchemy and the selected setup is Neon, PlanetScale, Prisma Postgres, or Aurora, provisioning defaults to `alchemy`.
