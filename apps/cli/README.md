...
| **Runtime**              | • Bun<br>• Node.js<br>• Cloudflare Workers<br>• AWS Lambda<br>• None                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| **Database Setup**       | • Turso (SQLite)<br>• Cloudflare D1 (SQLite)<br>• Neon (PostgreSQL)<br>• Supabase (PostgreSQL)<br>• Prisma Postgres<br>• MongoDB Atlas<br>• AWS Aurora (PostgreSQL/MySQL)<br>• None (manual setup)                                                                                                                                                                                                                                                                                                           |
...
Options:
  ...
  --payments <provider>           Payments provider (polar, stripe, autumn, dodo, creem, chargebee, commet, none)
  ...
  --db-setup <setup>              Database setup (turso, d1, neon, supabase, prisma-postgres, planetscale, aurora, mongodb-atlas, docker, none)
  --web-deploy <setup>            Web deployment (cloudflare, aws, docker, vercel, none)
  --server-deploy <setup>         Server deployment (cloudflare, aws, docker, vercel, none)
  ...
  --runtime <runtime>             Runtime (bun, node, workers, lambda, none)
...
- **Turnstile addon**: Requires `better-auth` and Cloudflare deployment (`--web-deploy cloudflare`, plus `--server-deploy cloudflare` or `--backend self`). It is creation-only and cannot be added to an existing project. Set `TURNSTILE_DOMAINS` in `.env` to the production hostname(s) before deploying.
