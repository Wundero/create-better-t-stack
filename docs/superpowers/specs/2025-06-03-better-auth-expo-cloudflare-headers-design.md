# Better Auth Expo Immutable Headers on Cloudflare Design

## Goal

Fix the "Can't modify immutable headers" error that occurs when Better Auth is used with the Expo plugin and the backend runs on Cloudflare Workers.
...
Web auth works; the failure appears only when requests come from the React Native/Expo client. The generated templates currently pass framework request objects directly into `auth.handler(...)` and often pass raw request headers directly into `auth.api.getSession(...)`. In Workers, these headers can be immutable.
...
### 2.1 Generated Helper

Location: `packages/template-generator/templates/auth/better-auth/server/base/src/lib/cloudflare-auth.ts.hbs`
...
### 4.1 Likely affected auth route templates

- Hono: `packages/template-generator/templates/backend/server/hono/src/index.ts.hbs`
- Elysia: `packages/template-generator/templates/backend/server/elysia/src/index.ts.hbs`
- Fastify: `packages/template-generator/templates/backend/server/fastify/src/index.ts.hbs`
- Next fullstack: `packages/template-generator/templates/auth/better-auth/fullstack/next/src/app/api/auth/[...all]/route.ts.hbs`
- TanStack Start fullstack: `packages/template-generator/templates/auth/better-auth/fullstack/tanstack-start/src/routes/api/auth/$.ts.hbs`
- Astro fullstack: `packages/template-generator/templates/auth/better-auth/fullstack/astro/src/pages/api/auth/[...all].ts.hbs`
- Nuxt fullstack: `packages/template-generator/templates/auth/better-auth/fullstack/nuxt/server/api/auth/[...all].ts.hbs`

### 4.2 Likely affected session/context templates

- tRPC context: `packages/template-generator/templates/api/trpc/server/src/context.ts.hbs`
- oRPC context: `packages/template-generator/templates/api/orpc/server/src/context.ts.hbs`
- Next dashboard: `packages/template-generator/templates/auth/better-auth/web/react/next/src/app/dashboard/page.tsx.hbs`
- TanStack Start middleware: `packages/template-generator/templates/auth/better-auth/web/react/tanstack-start/src/middleware/auth.ts.hbs`
- Other dashboard/middleware templates that pass `headers` into `auth.api.getSession`

### 4.3 Auth plugin logic
...
- If too broad, add: `apps/cli/test/better-auth-cloudflare.test.ts`
